...
```{note}
Viewsheds in QGIS are discussed in the section below.
```

## Visibility Analysis in QGIS

To conduct viewshed analysis in QGIS, install the Visibility Analysis plugin under Plugins -> Manage and Install Plugins. After installing the plugin, a Visibility analysis menu will appear in the Processing Toolbox (you might need to restart QGIS).

The first step is to run the Create viewpoints tool. All data should first be set to a projected/planar coordinate reference system. Input observer locations should be a point feature class. A radius can be set in meters, an observer height, and/or target height. Additional parameters can be set to limit the viewshed angle or azimuth.

The next step is to run the Viewshed tool. The Binary viewshed option will generate a traditional viewshed surface, and Addition will generate a cumulative viewshed. The Observer locations input will be the output of the Create viewpoints tool. The DEM should be defined, and the Take in account Earth curvature should be checked. The Depth below horizon option will create a surface showing the additional height needed for a location to become visible.
