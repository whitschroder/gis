...
The [Cell Statistics tool](https://pro.arcgis.com/en/pro-app/3.3/tool-reference/spatial-analyst/cell-statistics.htm) in ArcGIS Pro calculates a per cell statistic across multiple rasters.

```{note}
In QGIS, Cell Statistics is available under [Raster analysis](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html).
```

When the input rasters overlap, a statistic is applied at each cell value digital number to produce an output raster. When the rasters do not overlap, the rasters can be combined to increase the extent of data; for example, when combining several tiles of data (e.g., digital elevation models).
...
The Cell Statistics tool has a limited number of mathematical operations. The [Raster Calculator tool](https://pro.arcgis.com/en/pro-app/3.3/tool-reference/spatial-analyst/raster-calculator.htm) in ArcGIS Pro provides more flexibility in defining such operations.

```{note}
Refer to Raster Calculator in QGIS under [Raster analysis](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/rasteranalysis.html).
```
