...
However, the symbology here is misleading because, in reality, the raster image is rectangular. Under Symbology -> Mask, we can assign a background color (in this case black), which applies to No Data areas. 

```{note}
We can do the same in QGIS by right-clicking the layer in Layers, selecting Properties -> Transparency, and under NoData Value, setting a value and the display color for NoData.
```
...
So, in fact, the raster is rectangular. Areas outside of the region of interest have been assigned No Data. In addition, we can also see that some areas inside the region of interest have the same No Data value. These are areas where no elevation data is available. If we use the Explore tool in ArcGIS Pro to identify the values in these locations, we will see they are assigned NoData.

```{note}
In QGIS, use Identify Features.
```
...
```{note}
QGIS uses slightly different terminology, with Byte representing 8 Bit unsigned, Int8 representing 8 Bit signed, Int16 representing 16 Bit signed, UInt16 representing 16 Bit unsigned, and Float32 representing 32 Bit float.
```
