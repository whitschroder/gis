...
In ArcGIS Pro, the geometries of feature classes can be documented by using the Measure tool under Map -> Inquiry -> Measure Distance. 

```{note}
In QGIS, the Measure Line tool (and Measure Area) can be used. 
```

Here distances, perimeters, areas, and angles can be easily measured using the cursor. However, these values are not saved anywhere and would require manual copying and data entry to store them. Alternatively, we can add the geometry of vectors to their attribute table for further analysis.
...
The [Summarize Nearby tool](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/analysis/summarize-nearby.html) has similar functionality, but instead of summarizing data within another polygon, data are summarize within a buffer outside of the polygon, which can be defined with a Euclidean distance inside the tool itself.
