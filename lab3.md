...
4. Sites.shp -- a feature class of archaeological sites. For this study we used the [Witschey and Brown 2010 database](https://www.longwood.edu/staff/witscheywr) of known Maya sites. These sites were classified into three types: Type 1 representing the largest Maya sites, Type 2 representing smaller sites, and Type 3 representing the smallest sites.

5. soiltypes.csv -- a table with the same unique id numbers from centroids.dbf and the predominant soil type within the tile.

6. MEX_adm.shp -- a feature class of Mexican *municipios*, essentially equivalent to what we call counties in the U.S.

7. Structures.shp -- a feature class of annotated archaeological structures. These records represent architecture constructed above a platform or the ground surface.

8. Platforms.shp -- a feature class of annotated archaeological platforms. These records represent architecture that supports a structure.
...
2. The second map should show the state of Chiapas with the tiles or flight polygons in Chiapas, displayed with a continuous color scheme representing the density per square kilometer (count divided by DEM footprint area) of structures and platforms that have a footprint area greater than or equal to 20 square meters. Include labelled Type 1 and Type 2 archaeological sites. At this scale, the FlightPolygons.shp feature class should be used for this display rather than the demfootprint.shp, which will likely not be visible at smaller scales.

3. The third map should show the state of Yucatan with the tiles or flight polygons in Yucatan, displayed with a discrete/categorical color scheme representing the predominant soil type in each tile. Include labelled Type 1 and Type 2 archaeological sites. At this scale, the FlightPolygons.shp feature class should be used for this display rather than the demfootprint.shp, which will likely not be visible at smaller scales.

4. The fourth map should show the state of Campeche with the tiles or flight polygons in Campeche, displayed with a continuous color scheme representing the number of Type 1, Type 2, and Type 3 sites within 10 km of the boundary of each DEM footprint. Include labelled Type 1 archaeological sites with a 10 km buffer, and labelled Type 2 archaeological sites without a buffer. At this scale, the FlightPolygons.shp feature class should be used for this display rather than the demfootprint.shp, which will likely not be visible at smaller scales.
