...
The [Clip](https://pro.arcgis.com/en/pro-app/latest/tool-reference/analysis/clip.htm) tool in ArcGIS Pro or in QGIS under [Vector overlay](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/vectoroverlay.html) generates a new feature class that retains only those portions of a specified input layer that overlap with any one or more of the features on a specified "clip" layer.
...
The [Erase tool](https://pro.arcgis.com/en/pro-app/latest/tool-reference/analysis/erase.htm) in ArcGIS Pro (or in QGIS the Difference tool under [Vector overlay](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/vectoroverlay.html)) is essentially the opposite of the **Clip** tool. Where the **Clip** tool generates an output where the input and clip layers overlap, the **Erase** tool erases overlapping features and generates an output outside of the clip layer.
...
The **Merge** tool in the Arc GIS Pro Geoprocessing toolbox or in the QGIS Processing Toolbox should not be confused with the Merge tool available in the Editing tools (or Edit Geometry in QGIS). The latter Merge tool can be used to combine layers in similar fashion to the Geoprocessing **Merge** tool. However the Editing tool is more commonly used to merge multipart features into a single feature or attribute.
...
Another way to merge records into a single record is with the [Dissolve tool](https://pro.arcgis.com/en/pro-app/3.4/tool-reference/data-management/dissolve.htm) in ArcGIS Pro or in [Vector geometry](https://docs.qgis.org/3.44/en/docs/user_manual/processing_algs/qgis/vectorgeometry.html) in QGIS. This tool will take a set of records that share a field and combine them into a single record. These records will be grouped based on that shared field For example, a feature class of U.S. counties could be dissolved based on a column named "state" to create a feature class of U.S. states. Or a feature class of excavation units could be dissolved to create a feature class of excavation operations.
